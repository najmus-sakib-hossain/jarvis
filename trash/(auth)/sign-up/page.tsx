...
export default function SignUp() {
  ...
  return (
    <Card className="rounded-md max-w-md">      <CardHeader>
      <CardTitle className="text-lg md:text-xl">{lt("sign-up", "Sign Up")}</CardTitle>
      <CardDescription className="text-xs md:text-sm">
        {lt("enter-signup-details", "Enter your details to create an account.")}
      </CardDescription>
    </CardHeader>
      <CardContent className="py-0">
        <div className="grid gap-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              <Label htmlFor="first-name">{lt("first-name", "First name")}</Label>
              <Input
                id="first-name"
                placeholder={lt("first-name", "First name")}
                required
                onChange={(e) => {
          ...
          <div className="grid gap-2">
            <Label htmlFor="last-name">{lt("last-name", "Last name")}</Label>
            <Input
              id="last-name"
              placeholder={lt("last-name", "Last name")}
              required
              onChange={(e) => {
        ...
        <div className="grid gap-2">
          <Label htmlFor="email">{lt("email", "Email")}</Label>
            <Input
              id="email"
              type="email"
              placeholder={lt("email", "Email")}
              required
              onChange={(e) => {
          ...
          <div className="grid gap-2">
            <Label htmlFor="password">{lt("password", "Password")}</Label>
            <Input
              id="password"
              type="password"
              value={password}
              ...
              placeholder={lt("password", "Password")}
            />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="password_confirmation">{lt("confirm-password", "Confirm Password")}</Label>
            <Input
              ...
              autoComplete="new-password"
              placeholder={lt("confirm-password-placeholder", "Confirm your password")}
            />
          </div>          <div className="grid gap-2">            <Label htmlFor="image">{lt("profile-image-optional", "Profile Image (optional)")}</Label>
            <div className="flex items-end gap-4">
              {imagePreview && (
                <div className="relative w-16 h-16 rounded-sm overflow-hidden">
                  <Image
                    src={imagePreview}
                    alt={lt("profile-preview-alt", "Profile preview")}
                    layout="fill"
                    objectFit="cover"
                  />
                </div>
                ...
                await signUp.email({
                  ...
                  fetchOptions: {
                    onError: (ctx) => {
                      ...
                      toast.info(lt("sign-up-success", "Sign up successful!"));
                      setLoading(false);
                    },
                  },
                });
              } catch (error) {
                toast.error(lt("upload-failed", "Upload failed."));
                setLoading(false);
              }
            }}
          >            {loading ? (
            ...
            lt("create-account", "Create Account")
          )}
          </Button>
          <div className="w-full flex flex-row space-x-2 items-center">
            <Separator className="max-w-1/3" />
            <span className="text-center text-sm text-muted-foreground flex-1">{lt("or-continue-with", "Or continue with")}</span>
            <Separator className="max-w-1/3" />
          </div>
      ...
      <CardFooter className="py-0">
        <div className="flex justify-center w-full">
          <p className="text-center text-xs text-muted-foreground">
            {lt("already-have-account", "Already have an account?")}{" "}
            <Link href="/signin" className="hover:underline">
              <span className="text-primary">{lt("sign-in", "Sign In")}</span>
            </Link>
          </p>
        </div>
      </CardFooter>
